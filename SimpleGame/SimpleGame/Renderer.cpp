...
void Renderer::CreateParticleVBO()
{
	...
	for (int i = 0; i < particleCount; i++)
	{
		...
		randomValueX = float((rand() / (float)RAND_MAX - 0.5)*velocityScale);
		randomValueY = float((rand() / (float)RAND_MAX - 0.5)*velocityScale);
		randomValueZ = 1.f;
...
void Renderer::DrawText(float x, float y, void* font, float r, float g, float b, char* text)
{
	...
	glDisable(GL_BLEND);
	glWindowPos2i(GLint(x+m_WindowSizeX/2), GLint(y+m_WindowSizeY/2));
	glColor3f(r, g, b);
